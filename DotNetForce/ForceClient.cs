...
namespace DotNetForce.Force
{
    internal class ForceClient : IForceClient, IDisposable
    {
        ...
        public ForceClient(string instanceUrl, string accessToken, string apiVersion, JsonHttpClient jsonHttpClient, XmlHttpClient xmlHttpClient)
        {
            if (string.IsNullOrEmpty(instanceUrl)) throw new ArgumentNullException("instanceUrl");
            if (string.IsNullOrEmpty(accessToken)) throw new ArgumentNullException("accessToken");
            if (string.IsNullOrEmpty(apiVersion)) throw new ArgumentNullException("apiVersion");
            _jsonHttpClient = jsonHttpClient ?? throw new ArgumentNullException("jsonHttpClient");
            _xmlHttpClient = xmlHttpClient ?? throw new ArgumentNullException("xmlHttpClient");
        ...
        public Task<QueryResult<T>> QueryAsync<T>(string query)
        {
            ...
            return _jsonHttpClient.HttpGetAsync<QueryResult<T>>(string.Format("query?q={0}", DNF.EscapeDataString(query)));
        }
        ...
        public Task<QueryResult<T>> QueryAllAsync<T>(string query)
        {
            ...
            return _jsonHttpClient.HttpGetAsync<QueryResult<T>>(string.Format("queryAll/?q={0}", DNF.EscapeDataString(query)));
        }
        ...
        public Task<List<T>> SearchAsync<T>(string query)
        {
            ...
            return _jsonHttpClient.HttpGetAsync<List<T>>(string.Format("search?q={0}", DNF.EscapeDataString(query)));
        }
